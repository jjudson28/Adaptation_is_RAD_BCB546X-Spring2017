...
pops_grey <- allfoxes[,1] #subsets 
grey_foxes_pca <- prcomp(allfoxes[,2:4859], scale = TRUE) #runs PCA function on the 
summary(grey_foxes_pca)
gf_pc_scores <- data.frame(grey_foxes_pca$x)
allfoxplot <- ggplot(gf_pc_scores) + geom_point(aes(x = PC1, y = PC3, colour = pops_grey)) + coord_equal() + ggtitle("PCA Plot with Island Populations and Grey Fox") + labs(x = "PC1(18.3%)", y = "PC3(35.3%)")
...
summary(ng_foxes_pca)
ngf_pc_scores <- data.frame(ng_foxes_pca$x)
nogreyplot <- ggplot(ngf_pc_scores) + geom_point(aes(x = PC1, y = PC3, colour = pops_ng)) + coord_equal() + ggtitle("PCA Plot with Island Populations") + labs(x = "PC1(21.1%)", y = "PC3(39.9%)")
